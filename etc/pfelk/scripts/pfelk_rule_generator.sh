...
Script options:
	...
	${yellow}--auto-sync		${normal}Install or start the auto update script for ${red}OPNsense${normal}
...
auto-sync(){
	URL=https://raw.githubusercontent.com/ShagoY/pfelk/master/etc/pfelk/scripts/pfelk_rule_sync.sh
	FILE=/usr/local/opnsense/scripts/pfelk/pfelk_rule_sync.sh
	FILE2=/usr/local/opnsense/scripts/pfelk/pfelk_rule_sync.ini
	FILE3=/usr/local/etc/rc.syshook.d/update/20-sync-pfelk
	FILE4=/usr/local/opnsense/service/conf/actions.d/actions_filter.conf
	# Download the script pfelk_rule_sync.sh
	echo -e "Download ${yellow}pfelk_rule_sync.sh${normal} from github..."
	curl --create-dirs $URL -o $FILE
	echo -e "File location: ${yellow}$FILE${normal}"
	chmod +x $FILE
	if test -f "$FILE2"; then
		. $FILE2
		read -p "$(echo -e "Enter Your ${blue}pfELK${normal}'s IP Address/Hostname [default=${yellow}$pfelk_ip"${normal}]:) " answer
		: ${answer:=$pfelk_ip}
	else 
		read -p "Enter Your ${blue}pfELK${normal}'s IP Address/Hostname: " answer
	fi
	echo -e "Update the configuration file ${yellow}pfelk_rule_sync.ini${normal}"
	echo "pfelk_ip=$answer" > $FILE2
	# Updated file actions_filter.conf to start the script when modifying firewall rules
	echo -e "Backup the old ${yellow}actions_filter.conf${normal} file to ${yellow}actions_filter.conf.bak${normal}"
	cp $FILE4 $FILE4.bak
	echo -e "Updated file ${yellow}actions_filter.conf${normal} to integrate the pfELK script"
	sed -i '' 's/command:\/usr\/local\/etc\/rc\.filter_configure.*/command:\/usr\/local\/etc\/rc\.filter_configure;\/usr\/local\/opnsense\/scripts\/pfelk\/pfelk_rule_sync.sh/' $FILE4
	echo -e "Restart the ${yellow}configd service${normal}"
	service configd restart
	echo -e "Reload the ${yellow}OPNSense filter service${normal}"
